...
public final class JedisUtil {
     ...
     * Redis服务器IP
     */
    // private static String ADDR = "192.168.0.251";
     ...
     * 访问密码
     */
    // private static String AUTH = "fengniaosujia";
    ...
    static {
        try {
            ...
            props.load(JedisUtil.class.getClassLoader().getResourceAsStream("spring-redis.properties"));
            ...
            config.setMaxTotal(Integer.valueOf(props.getProperty("redis.maxActive")));
            ...
            jedisPool = new JedisPool(config, props.getProperty("redis.host"),
                    Integer.valueOf(props.getProperty("redis.port")), TIMEOUT, props.getProperty("redis.pass"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public String put(Object key, Object value) {
        ...
        try {
            jedis = jedisPool.getResource();
            return jedis.set(keyf.getBytes(), SerializeUtil.serialize(valuef));
        } catch (Exception e) {
            log.error("setObject设置redis键值异常:key=" + key + " value=" + value + " cause:" + e.getMessage());
            return null;
        } finally {
    ...
    public class SortSet {
        ...
        /*
         * public long zadd(String key, Map<Double, String> scoreMembers) { Jedis jedis = getJedis(); long s =
         * jedis.zadd(key, scoreMembers); returnJedis(jedis); return s; }
         */
    ...
    public class Strings {
        ...
        /**
         * @Title: getList @Description：(根据key以及类得到redis服务器中集合字符串对应的集合) @param @param key @param @param
        ...
        public <T> List<T> getList(String key, Class<T> clazz) {
            ...
            if (!isGoodJson(valueStr)) {
                valueStr = JsonUtils.toJson(valueStr);
            }
            return JsonUtils.parseArray(valueStr, clazz);
        }

        /**
         *
         * @Title: getObject @Description: (根据key以及类得到redis服务器中对象字符串对应的对象) @param @param key @param @param
        ...
        public <T> T getObject(String key, Class<T> clazz) {
            ...
            return (T) JsonUtils.parseArray(valueStr, clazz);
        }
    }
    ...
    /**
     *
     * @Title: isGoodJson @Description: (判断是否是json格式字符串) @param @param json @param @return 设定文件 @return boolean
     *         返回类型 @throws
     */
    ...
    /*
     * public static void main(String[] args) { JedisUtil jedisUtil = JedisUtil.getInstance(); JedisUtil.Strings strings
     * = jedisUtil.new Strings(); JedisUtil.Keys keys = jedisUtil.new Keys(); keys.rename("test", "testNew"); //
     * strings.set("nnn", "nnnn"); // System.out.println("-----" + strings.get("nnn")); // // Jedis jedis =
     * JedisUtil.getInstance().getJedis(); // for (int i = 0; i < 10; i++) { // jedis.set("test", "test"); //
     * System.out.println(i + "==" + jedis.get("test")); // // } // JedisUtil.getInstance().returnJedis(jedis); }
     */
    ...
    public static SortSet getSortSet() {
        return jedisUtil.new SortSet();
    }

    public static Sets getSets() {
        return jedisUtil.new Sets();
    }

    public static String customKeyGenerator(Object o, String method, Object...objects) {
